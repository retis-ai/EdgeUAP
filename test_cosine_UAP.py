...
alpha = config['alpha']
samples_mlp_target = 50
samples_mlp_others = 1000
samples_attack_dataset = config.get(config['samples_attack_dataset'], 1000)
num_max_mlp_training_epochs =  50
...
for NAME in MODEL_NAMES:
    ...
    for info in layers_info:
        ...
        if baseline_optimization is True:
            layer_name = 'all'
            uap, attack_results = utils_uap.run_baseline_inf_multi_layer_cosine(
                model,
                input_shape=224,
            ...
            uap, attack_results = utils_uap.run_attacks_inf_multi_layer_cosine(
                model,
